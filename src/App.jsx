...
export default function App() {
  ...
  const handleUndo = useCallback(() => {
    ...
    const { type, from, to, original, revised, at, length } = lastAiChange;
    if (type === 'replace') {
  ...
  return (
    <div className="flex flex-col h-screen" style={{ background: '#b0b0b0' }}>
      ...
      {editor && (
        <div
          className="text-xs text-right px-6 py-1.5 select-none"
          style={{ background: '#2B579A', color: 'rgba(255,255,255,0.7)', borderTop: '1px solid #1e4080' }}
        >
          {countWords(editor.getText())} ord
        </div>
      )}
